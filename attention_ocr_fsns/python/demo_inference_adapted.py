...
# manually adapt batch_size in 'common_flags.py' if not running via command line
...
def load_images(file_pattern, batch_size, dataset_name):
    # for debugging
  file_pattern = './testdata/fsns_train_%02d.png'
  ...
  for i in range(batch_size):
    ...
    # resize to bluerint fsns train images from creator
    # pil_image = cv2.resize(pil_image, (height,width))
    pil_image = pil_image.resize((width, height), PIL.Image.ANTIALIAS)
    # images_actual_data[i, ...] = np.asarray(pil_image)
    images_actual_data[i, ...] = tf.keras.preprocessing.image.img_to_array(pil_image)
